  ...
  async track(input: TrackAnalyticsEventInput) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "AnalyticsEvent"
        ("clientId", "sessionId", "eventName", "screenName", "interactionType", "target", "city", "language", "deviceType")
      VALUES
        (${input.clientId}, ${input.sessionId}, ${input.eventName}, ${input.screenName ?? null}, ${input.interactionType ?? null}, ${input.target ?? null}, ${input.city ?? null}, ${input.language ?? null}, ${input.deviceType ?? null})
    `;
  }
}
