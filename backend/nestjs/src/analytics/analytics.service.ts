...
export class AnalyticsService {
  ...
  async track(input: TrackAnalyticsEventInput) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "AnalyticsEvent" ("clientId", "sessionId", "eventName", "screenName", "interactionType", "target")
      VALUES (${input.clientId}, ${input.sessionId}, ${input.eventName}, ${input.screenName ?? null}, ${input.interactionType ?? null}, ${input.target ?? null})
    `;
  }
}
