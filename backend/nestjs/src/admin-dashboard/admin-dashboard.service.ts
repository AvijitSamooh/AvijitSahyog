  ...
  async getAnalytics() {
    const [audience, activity, trend] = await Promise.all([
      this.prisma.$queryRaw<Array<{ dau: bigint; wau: bigint; mau: bigint; newUsers: bigint; returningUsers: bigint }>>`
        WITH active AS (
          SELECT DISTINCT "clientId" FROM "AnalyticsEvent"
          WHERE "createdAt" >= NOW() - INTERVAL '30 days'
        ), first_seen AS (
          SELECT "clientId", MIN("createdAt") AS "firstSeenAt"
          FROM "AnalyticsEvent" GROUP BY "clientId"
        )
        SELECT
      ...
      this.prisma.$queryRaw<Array<{ sessions: bigint; screenViews: bigint; interactions: bigint; navigationEvents: bigint }>>`
        SELECT COUNT(DISTINCT "sessionId") AS sessions,
          COUNT(*) FILTER (WHERE "eventName" = 'screen_view') AS "screenViews",
          COUNT(*) FILTER (WHERE "eventName" = 'ui_interaction') AS interactions,
          COUNT(*) FILTER (WHERE "eventName" = 'navigation_select') AS "navigationEvents"
      ...
      this.prisma.$queryRaw<Array<{ day: Date; activeUsers: bigint; sessions: bigint; screenViews: bigint; interactions: bigint }>>`
        ...
        FROM "AnalyticsEvent" WHERE "createdAt" >= NOW() - INTERVAL '14 days'
    ...
    return {
      ...
      newUsers: Number(a.newUsers), returningUsers: Number(a.returningUsers),
      sessions: Number(b.sessions), screenViews: Number(b.screenViews),
      interactions: Number(b.interactions), navigationEvents: Number(b.navigationEvents),
      engagementTrend: trend.map((row) => ({
        date: row.day.toISOString().slice(0, 10),
        activeUsers: Number(row.activeUsers), sessions: Number(row.sessions),
        screenViews: Number(row.screenViews), interactions: Number(row.interactions),
      })),
    };
  }
