...
export class AdminDashboardService {
  ...
  async getAnalytics() {
    ...
    return {
      ...
      returningUsers: Number(a.returningUsers),
      sessions: Number(b.sessions),
      screenViews: Number(b.screenViews),
      interactions: Number(b.interactions),
      navigationEvents: Number(b.navigationEvents),
      engagementTrend: trend.map(r => ({date: r.day.toISOString().slice(0,10), activeUsers: Number(r.activeUsers), sessions: Number(r.sessions), screenViews: Number(r.screenViews), interactions: Number(r.interactions)})),
    };
  }
  ...
  async getAdvancedAnalytics() {
    const [retention, engagementCohorts, featureAdoption, language, device, city] = await Promise.all([
      this.prisma.$queryRaw<Array<{cohort: Date; users: bigint; day1: bigint; day7: bigint; day30: bigint}>>`
        WITH first_seen AS (
          SELECT "clientId", MIN("createdAt") AS first_seen
          FROM "AnalyticsEvent"
          GROUP BY "clientId"
        ), cohorts AS (
          SELECT "clientId", DATE_TRUNC('week', first_seen) AS cohort, first_seen
          FROM first_seen
      ...
      this.prisma.$queryRaw<Array<{cohort: Date; users: bigint; sessions: bigint; interactions: bigint}>>`
        SELECT DATE_TRUNC('week', "createdAt") AS cohort,
          ...
          COUNT(*) FILTER (WHERE "eventName" IN ('ui_interaction', 'navigation_select')) AS interactions
        FROM "AnalyticsEvent"
        WHERE "createdAt" >= NOW() - INTERVAL '12 weeks'
        GROUP BY DATE_TRUNC('week', "createdAt")
        ORDER BY cohort ASC`,
      this.prisma.$queryRaw<Array<{feature: string; users: bigint; events: bigint}>>`
        SELECT COALESCE(NULLIF("screenName", ''), 'unknown') AS feature,
          COUNT(DISTINCT "clientId") AS users,
          COUNT(*) AS events
        FROM "AnalyticsEvent"
        WHERE "createdAt" >= NOW() - INTERVAL '30 days' AND "eventName" = 'screen_view'
        GROUP BY COALESCE(NULLIF("screenName", ''), 'unknown')
        ORDER BY users DESC, feature ASC
        LIMIT 50`,
      this.prisma.$queryRaw<Array<{segment: string; users: bigint; events: bigint}>>`
        SELECT COALESCE(NULLIF("language", ''), 'unknown') AS segment, COUNT(DISTINCT "clientId") AS users, COUNT(*) AS events
        FROM "AnalyticsEvent" WHERE "createdAt" >= NOW() - INTERVAL '30 days'
        GROUP BY COALESCE(NULLIF("language", ''), 'unknown') ORDER BY users DESC, segment ASC`,
      this.prisma.$queryRaw<Array<{segment: string; users: bigint; events: bigint}>>`
        SELECT COALESCE(NULLIF("deviceType", ''), 'unknown') AS segment, COUNT(DISTINCT "clientId") AS users, COUNT(*) AS events
        FROM "AnalyticsEvent" WHERE "createdAt" >= NOW() - INTERVAL '30 days'
        GROUP BY COALESCE(NULLIF("deviceType", ''), 'unknown') ORDER BY users DESC, segment ASC`,
      this.prisma.$queryRaw<Array<{segment: string; users: bigint; events: bigint}>>`
        SELECT COALESCE(NULLIF("city", ''), 'unknown') AS segment, COUNT(DISTINCT "clientId") AS users, COUNT(*) AS events
        FROM "AnalyticsEvent" WHERE "createdAt" >= NOW() - INTERVAL '30 days'
        GROUP BY COALESCE(NULLIF("city", ''), 'unknown') ORDER BY users DESC, segment ASC LIMIT 100`,
    ]);

    const totalCohortUsers = retention.reduce((sum, row) => sum + Number(row.users), 0);
    ...
    return {
      retention: retention.map(row => ({
        ...
        day1Percent: row.users === 0n ? 0 : Math.round((Number(row.day1) / Number(row.users)) * 1000) / 10,
        day7Percent: row.users === 0n ? 0 : Math.round((Number(row.day7) / Number(row.users)) * 1000) / 10,
        day30Percent: row.users === 0n ? 0 : Math.round((Number(row.day30) / Number(row.users)) * 1000) / 10,
      })),
      engagementCohorts: engagementCohorts.map(row => ({
        cohort: row.cohort.toISOString().slice(0, 10),
        users: Number(row.users),
        sessions: Number(row.sessions),
        interactions: Number(row.interactions),
        sessionsPerUser: row.users === 0n ? 0 : Math.round((Number(row.sessions) / Number(row.users)) * 100) / 100,
      })),
      featureAdoption: featureAdoption.map(row => ({
        feature: row.feature,
        users: Number(row.users),
        events: Number(row.events),
        adoptionPercent: totalMau === 0 ? 0 : Math.round((Number(row.users) / totalMau) * 1000) / 10,
      })),
      segmentation: {
        city: segment(city),
      ...
      cohortUsers: totalCohortUsers,
      mau: totalMau,
      warehouse: {
        provider: 'postgresql',
        bigQueryReady: true,
        exportGrain: 'analytics_event',
      },
    };
  }
