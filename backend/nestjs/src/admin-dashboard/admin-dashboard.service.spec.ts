...
describe('AdminDashboardService', () => {
  ...
  it('maps analytics metrics and the daily engagement trend', async () => {
    prisma.$queryRaw
      .mockResolvedValueOnce([{ dau: BigInt(3), wau: BigInt(8), mau: BigInt(12), newUsers: BigInt(7), returningUsers: BigInt(5) }])
      .mockResolvedValueOnce([{ sessions: BigInt(20), screenViews: BigInt(80), interactions: BigInt(40), navigationEvents: BigInt(25) }])
      .mockResolvedValueOnce([{ day: new Date('2026-09-12T00:00:00.000Z'), activeUsers: BigInt(4), sessions: BigInt(6), screenViews: BigInt(18), interactions: BigInt(9) }]);

    await expect(service.getAnalytics()).resolves.toEqual({
      periodDays: 30, dau: 3, wau: 8, mau: 12, newUsers: 7, returningUsers: 5,
      sessions: 20, screenViews: 80, interactions: 40, navigationEvents: 25,
      engagementTrend: [{ date: '2026-09-12', activeUsers: 4, sessions: 6, screenViews: 18, interactions: 9 }],
    });
  });
});
