  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 32), children: [
        ...
        GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: _amounts.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.5), itemBuilder: (context, index) { final amount = _amounts[index]; return OutlinedButton(onPressed: () => _selectAmount(amount), style: OutlinedButton.styleFrom(backgroundColor: _selectedAmount == amount ? const Color(0xFFFCE8C9) : Colors.white), child: Text('₹ ' + amount.toString())); }),
        const SizedBox(height: 18),
        TextField(controller: _amountController, keyboardType: const TextInputType.numberWithOptions(decimal: true), onChanged: _onTotalChanged, decoration: InputDecoration(labelText: l10n.customAmount, prefixText: '₹ ', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)))),
        const SizedBox(height: 28),
        ...
        const SizedBox(height: 18),
        FilledButton.icon(onPressed: _submitting || _totalAmount <= 0 ? null : _submit, icon: _submitting ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.favorite_rounded), label: Text(l10n.donateNow)),
        const SizedBox(height: 12),
